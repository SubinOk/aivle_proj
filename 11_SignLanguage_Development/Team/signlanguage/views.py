...
def upload(request):
    if request.method == 'POST' and request.FILES['files1']:
        file_cnt = request.POST.get('file-cnt')
        result_list = []
        for idx in range(1, int(file_cnt)+1):

            # form에서 전송한 파일을 획득한다.
            file_id = 'files' + str(idx)
            answer_id = 'answer' + str(idx)
            ...
            # 모델 로딩
            SelModel =  AiModel.objects.get(is_selected = True)
            Model_name = SelModel.ai_file
            model_path = settings.MEDIA_ROOT + '/' + str(Model_name)
            model = load_model(model_path)

            # history 저장을 위해 객체에 담아서 DB에 저장한다.
            # 이때 파일시스템에 저장도 된다.
            result = Result()
            result.answer = request.POST.get(answer_id, '')
            result.image = file
            result.pub_date = timezone.datetime.now()
            result.save()

            modelcnt = ModelCnt()
            modelcnt.version = SelModel.version
            modelcnt.save()
